...
public class TabResultats implements TabTemplate {
	...
	private void maj (List<Vente> listVentes, HBox root) {
		// on créé une liste de produits en string
				String[][] topDaySalesString = new String[10][2];
				String[][] topMonthSalesString = new String[10][2];
				...
				for (int i=0; i<listVentes.size(); i++) {
					...
					LocalDate dateAchat = tempo.getDateAchat().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
	...
	private double getBenefice(Vente vente, Contrat contrat) {
		return (vente.getPrixDuMoment() * (vente.getQuantity() - contrat.getPrixFixe()));
	}
}
